...
export default function CardInspectorModal({ selectedCard, onClose }) {
  ...
  const mdBanStatus = 
    selectedCard.banlist?.ban_master_duel || 
    selectedCard.banlist?.master_duel || 
    selectedCard.banlist?.masterduel || 
    "Unlimited";

  const fullImageUrl = selectedCard.card_images?.[0]?.image_url || selectedCard.image;
